...
graph TD
    ...
    Node3 -->|Coordinates Found| Node4
    Node3 -->|Location Missing| Node5
...
* **`frontend/app.py`**: Chat client user interface. Implements sample query shortcuts and session history caching.
* **`backend/app/main.py`**: FastAPI server configuration and CORS middleware setups.
* **`backend/app/api/routes.py`**: Exposes `/api/chat` router wrapped in optional MLflow trace runs.
* **`backend/app/services/llm_service.py`**: Orchestration logic defining nodes, routing rules, guardrails, slot-extraction tools, and compiled state-graph compiler.
* **`backend/app/services/location_search.py`**: Performs local geocoding checks against provincial, municipal, and barangay coordinate lists.
...
* **`backend/app/models/schemas.py`**: Contains Pydantic models for incoming query validations and the conversational agent state.
...
1. **User Query**: User sends a prompt to Streamlit -> sent to `/api/chat` route.
...
3. **Intent Classification**: The `classifier` node routes the query into weather analytics, forecasts, or general chat intents.
4. **Slot Resolution**: The `tool_caller` node binds tool configurations and maps user questions to correct Open-Meteo variables.
...
6. **Fact-Grounded Generation**: The `generation` node passes the raw user question and Markdown context to DeepSeek, translating numbers into plain-language summaries under strict agricultural disclaimer rules.
